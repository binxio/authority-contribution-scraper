...
class BinxXkeSource(Source):
    ...
    def feed(self) -> Iterator[Contribution]:
        ...
        logging.info("reading new XKE sessions from https://xke.xebia.com/api/public since %s", latest)
        result = requests.get('https://xke.xebia.com/public/api/session/?unit=BINX')
        if result.status_code == 200:
            for session in result.json():
                date = datetime_parse(session['start_time']).astimezone(pytz.timezone("Europe/Amsterdam"))
                if date > latest:
                    presenters = map(lambda p: p.strip(), re.findall(r"[\w\s]+", session["presenter"]))
                    for presenter in presenters:
                        url = f"https://xke.xebia.com/event/{date.date()}/{session['id']}"
